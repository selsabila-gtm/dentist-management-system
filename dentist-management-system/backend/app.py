...
import os
import json
from datetime import datetime   # still used in seed_initial_data (for dates if needed)
...
class Appointment(db.Model):
    ...
    summary = db.relationship("Summary", back_populates="appointment")
    ...
    if Appointment.query.count() == 0:
        appts = [
            Appointment(
                ...
                patient_id=Patient.query.filter_by(first_name="Sophia").first().id
                if Patient.query.filter_by(first_name="Sophia").first()
                else None,
            ...
            Appointment(
                ...
                patient_id=Patient.query.filter_by(first_name="Ethan").first().id
                if Patient.query.filter_by(first_name="Ethan").first()
                else None,
...
# IMPORTANT: import routes AFTER app/db/models are defined
