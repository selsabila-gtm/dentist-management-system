...
# Replace the Patient model's to_dict() method in models.py
...
class Patient(db.Model):
    ...
    def to_dict(self):
        """Return patient data with proper field names for frontend compatibility"""
        # Ensure full_name is always available
        full_name = self.full_name or f"{self.first_name or ''} {self.last_name or ''}".strip()
        
        return {
            # Core identity fields
            "id": self.id,
            "name": full_name,  # Used by frontend lists
            
            # Name fields (camelCase for frontend)
            "firstName": self.first_name,
            "lastName": self.last_name,
            
            # Contact & demographics (camelCase for frontend)
            ...
            # Insurance (snake_case since that's what medical record endpoint expects)
